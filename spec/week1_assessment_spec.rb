...
describe BattleBot do
  ...
  describe "interactions" do
    ...
    describe '.count' do
      xit 'returns 0 if no bots have been created' do
        expect(BattleBot.count).to eq(0)
      end

      xit 'returns 1 if one bot has been created' do
        bot
        expect(BattleBot.count).to eq(1)
      end

      xit 'returns 2 if one bot has been created' do
        bot
        bot2
        expect(BattleBot.count).to eq(2)
      end

      xit 'returns only the number of LIVING bots' do
        bot
